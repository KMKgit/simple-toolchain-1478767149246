...
try:
  ...
  model = Sequential()
  model.add(LSTM(int(hidden_unit[0]), return_sequences=True, input_shape=(timesteps, data_dim)))
  model.add(Dropout(float(dropout[0])))
  
  for i in range(hidden_layer):
    if (i == hidden_layer-1):
      model.add(LSTM(int(hidden_unit[i+1])))
    else:
      model.add(LSTM(int(hidden_unit[i+1]), return_sequences=True))
