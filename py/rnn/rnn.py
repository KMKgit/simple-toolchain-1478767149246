...
try:
  ...
  with codecs.open(join(PATH + '/data/' + sys.argv[1] + '/' + sys.argv[1] + '.csv'), encoding = 'utf-8-sig') as csv_file:
      ...
      for i in range(samples):
        Y_train[i] = np.asarray(csv_data[i+1][label_index], dtype=np.int)
        temp = []
  ...
  model = Sequential()
  model.add(SimpleRNN(int(hidden_unit[0]), return_sequences=True, input_shape=(timesteps, data_dim)))
  model.add(Dropout(float(dropout[0])))
  
  for i in range(hidden_layer):
    if (i == hidden_layer-1):
      model.add(SimpleRNN(int(hidden_unit[i+1])))
    else:
      model.add(SimpleRNN(int(hidden_unit[i+1]), return_sequences=True))
